...
> **Early build, and still actively developed.** The core works and is played: targets lock, shots steer, companions can be supported. Some pieces are deliberately unfinished — most visibly there is no line-of-sight test yet, which is why automatic advancing to the next enemy ships switched off. Behaviour can change between builds, and the engine offsets are tied to one game version. [Status](#status) lists the known limits in full, and [Releases](https://github.com/alexanderjohnen/StarfieldVATS/releases) has the packaged builds.
